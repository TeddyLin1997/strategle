...
const ScrollerList = ({ title, list }: ScrollListProps) => {
  return (
    <div className="px-3">
      ...
      <section className="pt-4 pb-8 w-full flex items-center gap-3 overflow-auto snap-mandatory snap-x">
        {list.map(item => (
          <a key={`${item.source}-${item.title}`} href={item.url} target="_blank" className="pt-4 p-3 block flex-none basis-[calc(25%-0.75rem)] mg:basis-[calc(20%-0.75rem)] rounded-lg overflow-hidden shadow-lg bg-white snap-center hover:bg-up-extend hover:scale-110 transition-all">
